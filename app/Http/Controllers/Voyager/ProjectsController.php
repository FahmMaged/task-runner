...
class ProjectsController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($orderBy && (in_array($orderBy, $dataType->fields()) || !empty($row))) {
                ...
                $dataTypeContent = call_user_func([
                    $query->orderBy($orderBy, $querySortOrder)->with('tasks'),
                    $getter,
                ]);
            } elseif ($model->timestamps) {
                $dataTypeContent = call_user_func([$query->latest($model::CREATED_AT)->with('tasks'), $getter]);
            } else {
                $dataTypeContent = call_user_func([$query->orderBy($model->getKeyName(), 'DESC')->with('tasks'), $getter]);
            }
            ...
            $dataTypeContent = call_user_func([DB::table($dataType->name)->with('tasks'), $getter]);
            $model = false;
        }
        ...
        // Check if there are pending jobs to stop refreshing the page
        $jobs = Job::all();
        $refresh = 0;
        if(count($jobs)){
            $refresh = 1;
    ...
    public function show(Request $request, $id)
    {
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted', 'tasks', 'refresh'));
    }
