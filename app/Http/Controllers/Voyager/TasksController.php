...
class TasksController extends VoyagerBaseController
{
  ...
  // public function count()
    ...
    //         // Count lines
    //         $task->occurrences = count(file($path));
    //         $task->save();
    //         return count(file($path));
    //     }
    //     if($task->type == "Count Words"){
    //         // Count file words execluding the special characters
    //         $document    = file_get_contents($path);
    //         $words_array = preg_split("/[\s,]+/",$document);
    //         $task->occurrences = count($words_array);
    //         $task->save();
    ...
    //         $task->occurrences = strlen($document);
    //         $task->save();
    //         return strlen($document);
    //     }
  // }

    public function count($task)
    {
    ...
    public function store(Request $request)
    {
        ...
        if ($file[0]->getClientOriginalExtension() != 'txt') {
        ...
        $data->task_id = Str::random(20);
        $data->save();

        // $this->count($data);

        
        TaskRunner::dispatch($data);
        // dd($data);
